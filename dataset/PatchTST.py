...
class Dataset_TST_Benchpress(Dataset):
    def __init__(self, dataset_root, split_index=52):
        self.sample_paths = []  
        self.features = []
        self.labels = []
        ...
        for _, row in df.iterrows():
            ...
            data = row.iloc[:split_index].values.astype(float)
            label = [row.iloc[split_index+3]] + [row.iloc[split_index+2]] + row.iloc[split_index+4:split_index+6].values.astype(int).tolist()
            path = row.iloc[-1]
